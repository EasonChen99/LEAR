...
def train(args, TrainImgLoader, model, optimizer, scheduler, scaler, logger, device, epoch):
    ...
    for i_batch, sample in enumerate(TrainImgLoader):
        ...
        loss_edge = ClassifyLoss(depth_mask_bi, ground_truth_depth_mask, loss_func="Weighted_Cross_Entropy_Loss")
        ...
        loss_consist = ClassifyLoss(depth_mask_bi, ground_truth_event_mask, flow=-1 * flow_preds[-1], loss_func="Warped_Weighted_Cross_Entropy_Loss")
        metrics['consist_loss'] = loss_consist.item()
        # warped edge groundtruth consistence loss
        ground_truth_depth_mask_bi = ground_truth_depth_mask.unsqueeze(1).float()
        ground_truth_depth_mask_bi = torch.cat((1.-ground_truth_depth_mask_bi, ground_truth_depth_mask_bi), dim=1)
        loss_consist_2 = ClassifyLoss(ground_truth_depth_mask_bi, ground_truth_event_mask, flow=-1 * flow_preds[-1], loss_func="Warped_Weighted_Cross_Entropy_Loss")
        metrics['consist_loss_2'] = loss_consist_2.item()
        ## warped event mask consistence loss with edge prediction
        # ground_truth_event_mask = depth_mask_gt[:, 1, :, :].long()
        # cv2.imwrite(f'./visualization/input/{i_batch:05d}_event_mask_gt.png', (ground_truth_event_mask[0, ...].cpu().detach().numpy()* 255).astype(np.uint8))
        # loss_consist = ClassifyLoss(depth_mask_bi, ground_truth_event_mask.unsqueeze(1).float(), flow=flow_preds[-1], loss_func="Inverse_Warped_Weighted_Cross_Entropy_Loss")
        # metrics['consist_loss'] = loss_consist.item()
        # ## warped event mask consistence loss with edge groundtruth
        # ground_truth_event_mask_bi = ground_truth_event_mask.unsqueeze(1).float()
        # ground_truth_event_mask_bi = torch.cat((1.-ground_truth_event_mask_bi, ground_truth_event_mask_bi), dim=1)
        # loss_consist_2 = ClassifyLoss(ground_truth_event_mask_bi, ground_truth_depth_mask, flow=flow_preds[-1], loss_func="Warped_Weighted_Cross_Entropy_Loss")
        # metrics['consist_loss_2'] = loss_consist_2.item()

        alpha = 100
        beta = 100
        loss = loss_flow + alpha * loss_edge + beta * loss_consist + beta * loss_consist_2
...
def test(args, TestImgLoader, model, device, cal_pose=False):
    ...
    for i_batch, sample in enumerate(TestImgLoader):
        ...
        _, flow_up, depth_mask = model(lidar_input, event_input, iters=24, test_mode=True, idx=i_batch)
