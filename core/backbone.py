...
from core.extractor import BasicEncoder, ResidualBlock, Encoder_Edge_Fusion, Basic_Encoder_Edge_Fusion, Decoder_Edge
...
class Backbone_Edge_FF(nn.Module):
    ...
    def upsample_flow(self, flow, mask):
        """ Upsample flow field [H/8, W/8, 2] -> [H, W, 2] using convex combination """
        N, _, H, W = flow.shape
        mask = mask.view(N, 1, 9, 8, 8, H, W) # Bx576xH/8xW/8 -> Bx1x9x8x8xH/8xW/8
        mask = torch.softmax(mask, dim=2)

        up_flow = F.unfold(8 * flow, [3, 3], padding=1)
        up_flow = up_flow.view(N, 2, 9, 1, 1, H, W)

        up_flow = torch.sum(mask * up_flow, dim=2) # Bx2x9x8x8xH/8xW/8 -> Bx2x8x8xH/8xW/8
        up_flow = up_flow.permute(0, 1, 4, 2, 5, 3)
        return up_flow.reshape(N, 2, 8 * H, 8 * W)
