...
class Backbone_Edge_FF_Iter(nn.Module):
    def __init__(self, args):
        super(Backbone_Edge_FF_Iter, self).__init__()
        self.args = args
        self.hidden_dim = hdim = 128
        self.context_dim = cdim = 128
        args.corr_levels = 4
        ...
        # feature network, context network, and update block
        self.fnet_lidar = Encoder_Edge_Fusion_Iter(input_dim=1, output_dim=256, norm_fn='instance', dropout=args.dropout)
        self.fnet_event = BasicEncoder(input_dim=2, output_dim=256, norm_fn='instance', dropout=args.dropout)
        self.cnet = BasicEncoder(input_dim=1, output_dim=hdim + cdim, norm_fn='batch', dropout=args.dropout)
        self.update_block = BasicUpdateBlock(self.args, hidden_dim=hdim)
    ...
    def forward(self, image1, image2, edge_input, iters=12, flow_init=None, test_mode=False, idx=0):
        """ 
            Estimate optical flow between pair of frames 
            image1: lidar_input
            image2: event_frame
